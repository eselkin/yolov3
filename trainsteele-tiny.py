...
import test_steele  # Import test.py to get mAP after each epoch
from models import *
...
def train(
    ...
    # latest = weights + 'best-tiny-14-coco-100.pt'
    ...
    if resume:
        ...
        if cfg.endswith('yolov3.cfg'):
            ...
            cutoff = 15
        elif cfg.endswith('yolov4-tiny.cfg'):
            load_darknet_weights(model, weights + 'yolov3-tiny.conv.15')
            cutoff = 15
        model.to(device).train()
    ...
    t0 = time.time()
    model_info(model)
    # number of burn-in batches
    n_burnin = min(round(dataloader.nB / 5 + 1), 1000)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str,
                        default='cfg/yolov4-tiny.cfg', help='cfg file path')
    parser.add_argument('--data-cfg', type=str,
                        default='cfg/steele.data', help='coco.data file path')
